...
汇总本地记录的请求元数据：Token 数量、缓存读取、请求成功率、首 Token 延迟（TTFT）、输出速度、
提供方、账户和模型。
...
对应卡片才会显示数值；否则 TTFT 卡片会显示“暂无数据”，并说明需要客户端上报。
...
are populated only when the client reports those counters; otherwise TTFT shows “暂无数据 / No data”.
...
the core version; the current package version is `0.2.11`.
