...
Independent native Windows implementation. The project is intentionally separate from Electron and Python so migration can proceed feature-by-feature without disturbing the working version.
...
The C# app is independent from the Python and Electron versions. Its settings are stored at `%LOCALAPPDATA%\\BalancePet\\csharp-settings.json`; the token is protected in that file with Windows DPAPI. It has its own `assets` directory and does not read or write other version folders.
